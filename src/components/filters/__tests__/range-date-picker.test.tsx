...
import { RangeDatePicker, type RangeDatePickerProps } from "@/components/filters/range-date-picker";
...
jest.mock("@/lib/utils", () => {
    ...
    ): string => args.filter((v): v is string => typeof v === "string").join(" ");
    return { cn };
});
...
function setup(props?: Partial<RangeDatePickerProps>) {
    const onChange = jest.fn<void, [import("react-day-picker").DateRange | undefined]>();
