...
describe("<DatePicker />", () => {
  ...
  test("<DatePicker /> hides Clear button when showClear=false (Today still visible by default)", async () => {
    const user = userEvent.setup();
    const value = new Date("2025-06-10T14:30:00");
    ...
    render(
      <DatePicker
        value={value}
        onChange={() => {}}
        showClear={false} // hide only Clear
        // showToday defaults to true
      />,
    );

    // Open popover by clicking the trigger (its name is the formatted date)
    await user.click(
      screen.getByRole("button", { name: dfFormat(value, "PPP") }),
    );

    // Your PopoverContent uses a test id, not role="dialog"
    // (fallback to role if you later add it)
    const popover =
      (screen.queryByRole("dialog") as HTMLElement | null) ??
      (await screen.findByTestId("popover-content"));

    // Clear is absent
    expect(
      within(popover).queryByRole("button", { name: /clear/i }),
    ).toBeNull();

    // Today remains visible (default showToday=true)
    expect(
      within(popover).getByRole("button", { name: /today/i }),
    ).toBeInTheDocument();
  });
